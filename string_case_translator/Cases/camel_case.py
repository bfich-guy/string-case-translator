...
def camel_to_screaming(*, variable_name: str) -> str | None:
    ...
    """
    This function gets variable name writed in camelCase and **returns SCREAMING_CASE**. 
    If there is an TypeError, function **returns None**. 
    """
    ...
    try:
        ...
        for character in variable_name:
            character_is_in_upper_register: bool = character.isupper()
            uppered_character: str = character.upper()

            if character_is_in_upper_register:
                screaming_word_list.append(screaming_case_divider)
                screaming_word_list.append(uppered_character)
