...
def snake_to_screaming(*, variable_name: str) -> str | None:
    ...
    """
    This function gets variable name writed in snake_case and **returns SCREAMING_CASE**. 
    If there is an TypeError, function **returns None**. 
    """

    try:
        screaming_case_divider: str = "_"
        snake_word_list: list[str] = variable_name.split(screaming_case_divider)
        screaming_word_list: list[str] = []

        for snake_word in snake_word_list:
            screaming_word: str = snake_word.upper()
            screaming_word_list.append(screaming_word)

        result: str = screaming_case_divider.join(screaming_word_list)
        return result
    except TypeError:
        return None